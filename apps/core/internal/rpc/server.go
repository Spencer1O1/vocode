...
type Server struct {
	...
	writeMu sync.Mutex
	// next outbound requests share the same writer, so serialize.
...
// Request sends an outbound JSON-RPC request and blocks until the matching response is received.
...
// ApplyDirectives sends `host.applyDirectives` to the extension host and waits for the result.
func (s *Server) ApplyDirectives(params protocol.HostApplyParams) (protocol.HostApplyResult, error) {
...
// Run reads JSON-RPC messages line-by-line from stdin until EOF.
...
func (s *Server) encodeSuccess(id int64, result any) {
	...
	_ = s.encoder.Encode(protocol.JSONRPCResponse[any]{
		JSONRPC: jsonRPCVersion,
		ID:      id,
		Result:  result,
	})
...
func (s *Server) encodeError(id *int64, err *protocol.JSONRPCErrorObject) {
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	_ = s.encoder.Encode(protocol.JSONRPCErrorResponse{
		JSONRPC: jsonRPCVersion,
		ID:      id,
		Error:   *err,
	})
