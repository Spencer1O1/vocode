	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"search_query": map[string]any{
				"type":        "string",
				"description": "workspace_select: symbol/identifier name or exact literal substring to find in file contents. file_select: single file or folder basename only (no slashes, no absolute path). command, create_entry, create, move, rename, delete, control, irrelevant, question: always empty.",
			},
			"search_symbol_kind": map[string]any{
				"type":        "string",
				"description": "workspace_select only: optional kind of symbol they mean — function, method, class, variable, constant, interface, enum, property, field, constructor, module, struct, or type. Empty when unknown or for file_select.",
			},
		},
