// Package intents defines planner output validated by the daemon and routed by the intents
// dispatch layer (control vs executable).

// # Taxonomy

// Intents are either control or executable (see [Intent]).

// Control intents ([ControlIntent]) affect the planner loop only; they do not produce host
// directives. Payload types:
//  - [DoneIntent] — optional summary string for the extension UI when stopping ([ControlIntentKindDone]).
//  - [RequestContextIntent] — symbols, file excerpts, usage notes for the planning context.
