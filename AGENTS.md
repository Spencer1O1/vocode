...
1. **Core (`apps/core`, binary `vocode-cored`)**: reasoning + semantic safety policy + converting model output into validated, protocol-shaped results.
...
One "turn" starts when the extension calls the core JSON-RPC entrypoint:

`voice.transcript(text, activeFile?)` -> `VoiceTranscriptCompletion`

### What the core guarantees

1. For `voice.transcript`, the core runs a **narrow-model** path through **`internal/transcript/pipeline`** and **`internal/flows`** (classification -> flow-specific handlers -> scoped edit / format / rename / search / ...), not an iterative `intents[]` loop.
2. **Single-shot apply per utterance**: at most one `pipeline.Execute` pass and one `host.applyDirectives` batch for that RPC. On `stale_range` or other apply failures, the core returns failure; the user re-speaks (99-like); no core-side repair retries.
3. The core returns a `VoiceTranscriptCompletion` containing:
   - `success`
   - optional `summary`
   ...
   Per-`contextSessionId` state lives in `transcript/session.VoiceSessionStore`, subject to idle reset (`VOCODE_DAEMON_SESSION_IDLE_RESET_MS`: unset -> 30m default, `0` -> off; prefix is historical) and rolling byte/excerpt cap (`daemonConfig` on the RPC: `maxGatheredBytes`, `maxGatheredExcerpts`).
4. Each directive is exactly one of:
   - `kind: "edit"` with an `editDirective` (a single explicit variant of `EditDirective`)
   - `kind: "command"` with `commandDirective` (core-validated command shape; extension executes)
...
2. For `edit` directives, it applies core-provided edit actions mechanically using `workspace.applyEdit`.
...
### Invariant: no core "execution RPCs"
...
- **Policy-free constructors and validators**: `packages/protocol` must only express wire shape and schema invariants. No core/extension policy (allowlists, safety semantics, env defaults, retries) in validators or `go/*_constructors.go` helpers; see `packages/protocol/README.md`.
...
- Do not add core-side execution calls to run shell commands. The core should validate and return `commandDirective`; the extension executes.
- Do not add new RPC endpoints like `edit.dispatch` or `command.run`. Keep the main entrypoint as `voice.transcript`.
- Do not move semantic policy logic into the extension (keep it in core-owned layers).
...
## Core package boundaries (voice transcript)

- **`internal/transcript` (root `transcript` package):** `Service` — RPC-facing facade, bounded queue, mutex; each utterance runs `pipeline.Execute` under the lock (or a fast path for control requests).
- **`internal/transcript/pipeline`:** session load, control requests, clarify overlay, phase dispatch, narrow model calls, `host.applyDirectives`, persistence.
- **`internal/transcript/run`:** `Env` wiring (sessions, flow router, host apply, search).
- **`internal/transcript/session`:** `VoiceSession`, store, clone/idle helpers.
- **`internal/flows` + `internal/flows/router`:** flow IDs, route classification, per-flow dispatch (`root`, `global`, `workspaceselect`, `fileselect`, ...).
- **`internal/agent`:** model client (`ClassifyTranscript`, `ScopeIntent`, `ScopedEdit`, ...) and provider adapters.
- Target dependency direction: `transcript` -> `pipeline` -> `flows` / `agent` / `workspace` / `session` (acyclic; avoid import cycles — follow existing imports).

## Where to look (key files)

### Core (`apps/core`)

- Agent/runtime: `internal/agent` — transcript/scoped-edit model calls. Providers: `openai` (`VOCODE_AGENT_PROVIDER=openai`, `OPENAI_API_KEY`, optional `VOCODE_OPENAI_MODEL` / `VOCODE_OPENAI_BASE_URL`); `anthropic` (`VOCODE_AGENT_PROVIDER=anthropic`, `ANTHROPIC_API_KEY`, optional `VOCODE_ANTHROPIC_MODEL` / `VOCODE_ANTHROPIC_BASE_URL`). Transcript debug: `VOCODE_DAEMON_VOICE_LOG_TRANSCRIPT=1` (historical env name; core stderr). Workspace paths + file ops: `internal/workspace`. Search (ripgrep): `internal/search`. Transcript search wiring: `internal/transcript/searchapply`.
- Voice transcript: `internal/transcript/service.go` (queue, RPC); `internal/transcript/pipeline` (execute + phases); `internal/transcript/clarify`, `controlrequest`, `hostdirectives`, `outcome`, `idle`.

### Extension

- Send transcript: `apps/vscode-extension/src/commands/send-transcript.ts`
- Apply directive batches + return per-directive outcomes: `apps/vscode-extension/src/voice-transcript/apply-directives.ts`, `apps/vscode-extension/src/daemon/rpc-transport.ts` (handler for `host.applyDirectives`; folder name `daemon/` is historical)
...
- JSON-RPC client for `vocode-cored`: `apps/vscode-extension/src/daemon/client.ts`
- Voice sidecar spawn/client: `apps/vscode-extension/src/voice` (`client`, `spawn`, `paths`)
- Spawned core/voice env: `apps/vscode-extension/src/config/spawn-env.ts` — `package.json` configuration defaults + effective VS Code `vocode.*` settings + ElevenLabs key from SecretStorage (no workspace `.env`). **API keys** for cloud models are **not** in settings: set `OPENAI_API_KEY` and/or `ANTHROPIC_API_KEY` in your environment when using `vocode.daemonAgentProvider` `openai` or `anthropic` (setting id is historical). For `go run` / shell workflows, export vars yourself; default numbers/strings match `package.json` where applicable.
...
- STT adapter(s): `apps/voice/internal/stt`
  ...
  Rule: downstream STT must eventually produce transcript text, which is sent to the core via `voice.transcript`.

## Extending the system (checklist)

### Add a new edit capability

1. Extend `internal/agent` prompts/parsing and the transcript/flows path that emits the new edit shape.
...
3. Add tests in the owning layers:
   - core: transcript/pipeline/flows tests for the new path
...
2. Update core command validation / allowlist where it lives (flows or helpers).
3. Update extension allowlist in `apps/vscode-extension/src/directives/command/execute-command.ts`.
4. Keep execution semantics in the extension; keep command-shape validation in the core.
...
- If failure semantics change, update both core batch construction and extension stop-on-failure behavior.
...
3. If it talks to the core, call client methods only (no core policy logic in the command).
4. Add/extend extension tests for the new command behavior.

Rules:

- Keep command logic orchestration/UI-level.
- Any semantic safety logic belongs in core-owned layers.
...
4. Add a handler under `apps/core/internal/rpc`:
   - decode params
   - call one core entrypoint (thin handler)
   - return result or structured RPC error
5. Update the extension client and command usage.
6. Add RPC-level tests for success/failure/noop and invalid-result rejection when invariants exist.

Rules:

- Handlers must stay thin.
- Transport must not run the transcript pipeline or interpret model output.
- If a request crosses multiple core domains, route through app-level orchestration.
...
4. Implement core action building/validation in the transcript/flows layers that produce `EditDirective`.
5. Implement extension mechanical apply logic for the new action kind.
6. Add tests:
   - core action-building + validation tests
   - extension action-application tests
   - protocol validator acceptance/rejection tests

Rules:

- Core decides whether an action is safe/valid to emit.
- Extension only applies actions deterministically (no semantic policy).

### Add a new voice-scoped edit capability

1. Extend agent prompt + structured parsing in `apps/core/internal/agent`.
2. Map parsed output to `EditDirective` / actions in the owning transcript or flow package.
3. Add transcript/pipeline tests for supported/unsupported instruction expectations and failure reasons.
...
- Transcript layers build protocol `EditDirective` / other directives for `host.applyDirectives` (not a separate edit RPC).
...
- Core repair retries on `stale_range` inside a single `voice.transcript` RPC (single-shot only).
- Extension re-deciding semantic policy already owned by core.
- Ambiguous/overloaded result shapes (breaks runtime validators and tests).
- Adding temporary policy logic in the extension to unblock core work.
...
- Transcript/pipeline tests: directive building and safety validation behavior.
